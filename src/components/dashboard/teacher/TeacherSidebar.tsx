...
import { Home, Users, BookOpen, ChevronLeft, ChevronRight, Menu } from 'lucide-react'
import { useAppStore } from '../../../store/useAppStore'

export type TeacherSectionId = 'home' | 'students' | 'lessons'

interface TeacherSidebarProps {
  isOpen: boolean
  onToggle: () => void
  activeSection: TeacherSectionId
  onSelectSection: (section: TeacherSectionId) => void
}

const NAV_ITEMS: { id: TeacherSectionId; label: string; icon: typeof Home; subtitle: string }[] = [
  { id: 'home', label: 'HOME', icon: Home, subtitle: 'Educator Overview' },
  { id: 'students', label: 'STUDENTS', icon: Users, subtitle: 'Class Roster & Accuracy' },
  { id: 'lessons', label: 'LESSONS', icon: BookOpen, subtitle: 'Curriculum & Info Input' },
]
      ...
      <motion.aside
        ...
        className={`fixed left-0 top-0 bottom-0 z-40 flex flex-col justify-between py-6 px-3 border-r theme-transition select-none backdrop-blur-2xl ${
          isDarkMode
            ...
            : 'bg-white/75 border-black/[0.06] text-[#171717] shadow-editorial'
        }`}
      >
        ...
        <div className="space-y-8">
          ...
          {/* ── Sidebar Navigation List (Exactly 3 items: HOME, STUDENTS, LESSONS) ── */}
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3.5 px-3.5 py-3 rounded-2xl text-left transition-all duration-200 cursor-pointer relative ${
                    isActive
                      ? isDarkMode
                        ? 'bg-white/[0.08] text-violet-400 border border-violet-400/30 shadow-glow-cyan-subtle'
                        : 'bg-white/90 text-violet-800 border border-violet-400/40 shadow-subtle'
                      : isDarkMode
                ...
                >
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="activeTeacherNavIndicator"
                      className="absolute left-1 top-2.5 bottom-2.5 w-1 rounded-full bg-violet-500"
                    />
                  )}

                  <div className="shrink-0 flex items-center justify-center">
                    <Icon size={19} className={isActive ? 'text-violet-500' : ''} />
                  </div>
        ...
        <div className="px-2 pt-4 border-t border-black/[0.05] dark:border-white/[0.06]">
          {isOpen ? (
            <div className="flex items-center justify-between text-[11px] text-[#A3A39E]">
              ...
              <span className="text-[10px] font-semibold text-violet-500 bg-violet-500/10 px-2 py-0.5 rounded-full">
                EDUCATOR
              </span>
            </div>
          ) : (
            <div className="w-full flex justify-center">
              <button
                onClick={onToggle}
                className="text-[#A3A39E] hover:text-violet-500 p-1 cursor-pointer"
                title="Expand"
              >
